...
class section():
    def __init__(self, i0, j0, subImg, Params):
        ...
        self.meanFFT = np.empty([self.subParams['subSize'][0], self.subParams['subSize'][1], 3], dtype=np.complex64)
        self.meanNum = 0
        self.subParams['fRApprox'] = np.empty([3])
        ...
        for i in range(0, 3):
            ...
            self.subParams['coords'][i, :, :, :] = self.initCoords(i0, j0, self.subParams['wLen'][i])
    ...
    def initS0(self, img, size):
        ...
        FI0 = fftshift(FI0) #/ FI0.shape[0]**2
