...
def FPM(section, subImg):
    for i in range(3):
        ...
        S = section.S[:, :, i]
        P = section.P[:, :, i]
        bayer = section.bayer[:, :, i]
        invBayer = section.invBayer[:, :, i]
        LED = subImg.LEDPos

        dx, dy = section.subParams['coords'][i, LED[0], LED[1], 0] - section.subParams['subSize'][0] / 2, \
                 section.subParams['coords'][i, LED[0], LED[1], 1] - section.subParams['subSize'][0] / 2
